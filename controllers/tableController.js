...
const getAllTables = async (req, res) => {
  const tables = await Table.find({ createdBy: req.user.userId }).sort(
    "createdAt"
  );
...
const updateTable = async (req, res) => {
  ...
  const table = await Table.findByIdAndUpdate(
    ...
    {
      $set: {// este método é o mais correto? Altera todos os subdocs
        processes: {
          index: processos.index,
          code: processos.code,
          distance: processos.distance,
          factor: processos.factor,
          type: processos.type,
        },
      },
    },
